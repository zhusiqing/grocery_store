...
const checkVersion = (publishPackages, version) => {
  publishPackages.forEach(publishPackage => {
    ...
    const tags = tagHistory.split('\n').filter(el => {
      ...
      //   return ~el.indexOf(publishPackage.name)
      // }
      // return ~el.indexOf(publishPackage.name) && !~el.indexOf('beta')
    })
    tags.sort((a, b) => {
      const parseA = semver.parse(a.replace(publishPackage.name + '@', ''))
      const parseB = semver.parse(b.replace(publishPackage.name + '@', ''))
      const numA = parseA.major * 1000000 + parseA.minor * 10000 + parseA.patch * 100 + parseA.prerelease[1] || 0
      const numB = parseB.major * 1000000 + parseB.minor * 10000 + parseB.patch * 100 + parseB.prerelease[1] || 0
      return numB - numA
    })
    ...
    if (isBeta) {
      if (parsePreVersion.prerelease.length) {
        ...
        newVersion = `${parsePreVersion.major}.${parsePreVersion.minor}.${parsePreVersion.patch}-${parsePreVersion.prerelease.join('.')}`
      } else {
        newVersion = `${parsePreVersion.major}.${parsePreVersion.minor}.${parsePreVersion.patch}-beta.0`
      }
    } else {
      const versionArr = [parsePreVersion.major, parsePreVersion.minor, parsePreVersion.patch]
      if (!parsePreVersion.prerelease.length) {
        switch (version) {
          case 'patch':
...
const npmPublish = (publishPackages) => {
  console.log(publishPackages.length)
  publishPackages.forEach(publishPackage => {
    execSync(`cd ${publishPackage.path} && ls -al && npm publish`, { stdio: 'inherit' })
  })
}
...
const gitCommit = (publishPackages) => {
  const log = publishPackages.map(el => ` - ${el.name}@${el.newVersion}`).join('\n')
